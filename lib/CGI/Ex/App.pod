...
Can be used as a native PSGI app:
...
could simply let each "runmode" call the run_step hook of CGI::Ex::App
...
stored.  Default value is $ENV{SCRIPT_NAME} with path and extension removed.  You
will need to override this if $ENV{SCRIPT_NAME} is explicitly empty.
...
making your module into a PSGI app or a mod_perl handler.
...
Often in your program you will want to set cookies or bounce to a different URL.
...
is also very easy to run the previous example as a PSGI app.

PSGI support is optional and depends on L<CGI::PSGI>, so make sure you have
that module installed.

First, create a PSGI file:

    ### File: /var/www/recipe.psgi
    ### --------------------------------------------
    use lib qw(/var/www/lib);
    use Recipe;

    Recipe->to_app(name_module => 'recipe');

Then run it with L<plackup>:

    ### command-line or init script
    plackup /var/www/recipe.psgi

Alternatively, you can run it as a one-liner without the PSGI file:

    plackup -I/var/www/lib -MRecipe -e 'Recipe->to_app(name_module => "recipe")'

Any parameters passed to C<to_app> are simply forwarded to the app's C<new>
method upon construction.  Here we explicitly set name_module because
script_name will be empty and you would otherwise get this error: Could not
determine module name from "name_module".  Rather than pass the name_module
through C<to_app>, you could also override the name_module hook in the module.

    ### Add to /var/www/lib/Recipe.pm
    sub name_module { 'recipe '}

If you wanted to run your app at a different URL or run multiple apps in the
same server at different URLs, you could use L<Plack::App::URLMap>.  Here is
an example using L<Plack::Builder>, a DSL layer over URLMap:
    ...
    use Plack::Builder;
    builder {
        mount '/recipe' => Recipe->to_app;
        mount '/' => sub { [200, ['Content-Type' => 'text/plain'], ['Hi.']] };
    };

To convert an old legacy CGI script to a PSGI app without modifying the
original script, you can repackage it:
    ...
    sub main_file_print { \ 'Hello world' }

    __PACKAGE__->navigate;
    exit;

    ### command-line or init script
    plackup -MCGI::Ex::App -e 'CGI::Ex::App->repackage("/var/www/cgi-bin/myscript.pl")->to_app(name_module => "myscript")'
